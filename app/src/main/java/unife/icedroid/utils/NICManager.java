...
public class NICManager {
    ...
    public static void startWifiAdhoc() throws WifiAdhocImpossibleToEnable {
        try {
            ...
            cmd = "iplink set " + Settings.NW_IF + " down";
            Utils.rootExec(cmd);

            //Set wifi ad-hoc mode
            cmd = "iwconfig " + Settings.NW_IF + " mode ad-hoc channel " + Settings.NW_CHANNEL +
                  " essid " + Settings.NW_ESSID;
            Utils.rootExec(cmd);

            //Pull up wifi interface
            cmd = "iplink set " + Settings.NW_IF + " up";
            Utils.rootExec(cmd);

            //Set IP address and network settings
            cmd = "ipaddr add " + Settings.getIPAddress() + Settings.NW_MASK +
                  " broadcast " + Settings.NW_BROADCAST_ADDRESS + " dev " + Settings.NW_IF;
            Utils.rootExec(cmd);
